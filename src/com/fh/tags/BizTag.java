...
import com.fh.entity.biz.StationQuery.Criteria;
	...
	public void doTag() throws JspException, IOException {
		if (code != null && !"".equals(code)) {
			...
			if ("duty".equals(type)) {
				...
				Criteria criteria = stationQuery.createCriteria();
				if (!"all".equals(code)) {
					criteria.andStationCodeEqualTo(code);
				}
				if (StringUtil.isNotBlank(pcode)) {
					criteria.andDistrictCodeEqualTo(pcode);
				}
				...
				if (!"all".equals(code)) {
					...
					organiseCOQuery.createCriteria().andPOrganiseIdEqualTo(
							"Z001");
				}
				// DistrictDao districtDao = (DistrictDao)
				// applicationContext.getBean("districtDao");
				OrganiseCODao organiseCODao = (OrganiseCODao) applicationContext
						.getBean("organiseCODao");
				// List<District> districtList =
				// districtDao.selectByExample(districtQuery);
				List<OrganiseCO> organiseCOList = organiseCODao
						.selectByExample(organiseCOQuery);
				...
				DepPartDao depPartDao = (DepPartDao) applicationContext
						.getBean("depPartDao");
				List<DepPart> depPartList = depPartDao
						.selectByExample(depPartQuery);
