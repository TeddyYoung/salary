...
public class SalaryDifferenceServiceImpl implements SalaryDifferenceService {
	...
	public Page findSalaryDifferencesByPage(Page page, String staffName, String yearMonth, String subOrganiseIdStr) {
		...
		int totalRecordsNum = salaryDifferenceDao.findSalaryDifferenceCountByCriteriaQuery(staffName, yearMonth);
		page.setTotalRecordsNum(totalRecordsNum);
		// 分页查询记录
		List<SalaryDifference> records = salaryDifferenceDao.findSalaryDifferencesByPageCriteriaQuery(staffName, yearMonth,
																									  subOrganiseIdStr,
																						              page.getPageSize(), 
																						              page.getStartIndex());
	 ...
	 * 这是油站会计干的事, 别的人别瞎掺和
	 */
	public boolean saveOrUpdateSalaryDifference(List<SalaryDifference> salaryDifferenceCheckedList, 
											    HttpServletRequest request) {
		
		//判断数据库中是否有正在执行的流程
		//如果有, 不予提交申请
		int sdcount = salaryDifferenceDao.findSalaryDifferenceCountByApprovalStatus();
		if (0 == sdcount) {
			...
			if ("S_003".equals(userStorePart.getStorePart())) { //确定申请的人是油站会计, 开始干活
				...
				for (SalaryDifference salaryDifference : salaryDifferenceCheckedList) {
					if (null != salaryDifference.getId() && !"".equals(salaryDifference.getId())) {
						salaryDifference.setSysUpdateTime(new Date());
						salaryDifferenceDao.updateByPrimaryKeySelective(salaryDifference);
					}else{
						int count = salaryDifferenceDao.findSalaryDifferenceCountByCriteriaQuery(null, null);
						String salaryDifferenceCode = null;
						if (count < 9) {
							salaryDifferenceCode = "SD_000" + String.valueOf((count + 1));
						}else if (count >= 9 && count<= 98) {
							salaryDifferenceCode = "SD_00" + String.valueOf((count + 1));
						}else if (count >= 99 && count<= 998){
							salaryDifferenceCode = "SD_0" + String.valueOf((count + 1));
						}else if (count >= 999) {
							salaryDifferenceCode = "SD_" + String.valueOf((count + 1));
						}
						salaryDifference.setSalaryDifferenceCode(salaryDifferenceCode);
...
//							departmentCode = organiseCO.getpOrganiseId();
	...
	public SalaryDifference findSalaryDifferenceByStaffCode(String staffCode) {
		...
		if (null != staffCode && !"".equals(staffCode)) {
			...
			salaryDifferenceQuery.createCriteria().andStaffCodeEqualTo(staffCode);
			return salaryDifferenceDao.selectByExample(salaryDifferenceQuery).get(0);
		}else{
			throw new RuntimeException("The staffCode is null or '', we can't select anything by this staffCode!");
		}
	...
	public List<SalaryDifference> findStaffListWithStationNameByStationCode(String subOrganiseIdStr, String yearMonth) {
		...
		for (String organiseId : organiseIds) {
			List<SalaryDifference> salaryDifferences = salaryDifferenceDao.findStaffListWithStationNameByStationCode(organiseId, yearMonth);
	...
	public SalaryDifference findSalaryDifferenceWithStationNameByStaffCode(String staffCode) {
		...
		if (null != staffCode && !"".equals(staffCode)) {
			return salaryDifferenceDao.findSalaryDifferenceWithStationNameByStaffCode(staffCode);
		}else{
			return null;
		}
