...
public class ChallengeBonusServiceImpl implements ChallengeBonusService {
	...
	public Page findChallengeBonusPage(Page page, ChallengeBonus challengeBonus) {
		...
		int totalRecordsNum = challengeBonusDao.findCount(challengeBonus);
		page.setTotalRecordsNum(totalRecordsNum);
		// 分页查询记录
		List<ChallengeBonus> records = challengeBonusDao.findChallengeBonusPage(challengeBonus.getStationCode(),challengeBonus.getType(), page.getPageSize(), page.getStartIndex());
	...
	public void saveOrUpdate(ChallengeBonus challengeBonus) {
		if (null != challengeBonus.getId() && !"".equals(challengeBonus.getId())) {
	...
	public void updateList(List<ChallengeBonus> challengeBonusList) {
		for (ChallengeBonus challengeBonus : challengeBonusList) {
			List<ChallengeBonus> challengeBonusListTemp = this.challengeBonusDao.findChallengeBonusList(challengeBonus);
			if (challengeBonusListTemp != null && challengeBonusListTemp.size() > 0) {// 更新
				ChallengeBonus challengeBonusTemp = challengeBonusListTemp.get(0);
