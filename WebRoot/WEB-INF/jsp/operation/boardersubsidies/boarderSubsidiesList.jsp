...
<script type="text/javascript">
	...
	//条件查询
	function searchBoarderSubsidies() {
		$("#boarderSubsidiesForm").submit();
		$("#zhongxin").hide();
		$("#zhongxin2").show();
	}
	
	//导入Excel
	function importBoarderSubsidies() {
		bootbox.confirm("当前上传的Excel表格会自动将数据录入至上个月的数据中，请确认您上传的Excel表格是上个月的数据！", function(result) {
			if(result) {
				$("#boarderSubsidiesForm").attr("action", "<%=basePath%>boarderSubsidies/importBoarderSubsidies.do");
				$("#boarderSubsidiesForm").submit();
			}
		});
	}
	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>boarderSubsidies/boarderSubsidiesList.do"
						  name="boarderSubsidiesForm" id="boarderSubsidiesForm" method="post" enctype="multipart/form-data">
						...
						<table>
							<tr>
								...
								<td style=""><a 
									class="btn btn-mini btn-info" onclick="searchBoarderSubsidies();" style="margin-top:-11px;">查询</a></td>
								<td>
									<input type="file" name="uploadFile" />
									<a class="btn btn-mini btn-info" onclick="importBoarderSubsidies();" style="margin-top:-11px;">导入Excel数据</a>
								</td>
								<td>
									<a class="btn btn-mini btn-info" href="<%=basePath%>common/downloadTemplate.do?fileType=1" style="margin-top:-11px;">模板下载</a>
								</td>
							</tr>
						</table>
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class='center'>伙食方式</th>
									<th class='center'>搭伙补贴</th>
									<th class='center'>搭伙补贴备注</th>
									...
									<c:when test="${!empty pageList.records}">
										<c:forEach items="${pageList.records}" var="stationTarget"
											...
											<tr>
												...
												<td class="center">
<%-- 													<c:if test="${stationTarget.foodWay} != null"> --%>
														${stationTarget.foodWay}
<%-- 													</c:if> --%>
												</td>
												<td class="center">${stationTarget.boarderSubsidies}</td>
												<td class="center">
<%-- 													<c:if test="${stationTarget.boarderSubsidiesRemark} != null"> --%>
														${stationTarget.boarderSubsidiesRemark}
<%-- 													</c:if> --%>
												</td>
