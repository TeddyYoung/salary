...
<script type="text/javascript">
	...
	function importManageBase() {
		bootbox.confirm("当前上传的Excel表格会自动将数据录入至上个月的数据中，请确认您上传的Excel表格是上个月的数据！", function(result) {
			if(result) {
				$("#manageBaseForm").attr("action", "<%=basePath%>otherBonus/importLossBonus.do");
				$("#manageBaseForm").submit();
			}
		});
	}
						...
						<table>
							<tr>
								...
								<td style="vertical-align:top;">
									<select name="type" id="type"  class="chzn-select" data-placeholder="奖金类型" 
										style="vertical-align:top;width: 150px;" title="奖金类型">
										<option value=""></option>
										<systab:dataDictionary codeType="bonus_type" valueType="all">
											<option value="${dataDictionary.valuetype}"  ${dataDictionary.valuetype==lossBonus.type ? 'selected="selected"' : '' }>${dataDictionary.valuename}</option>
								...
								<td>
									<a class="btn btn-mini btn-info" href="<%=basePath%>common/downloadTemplate.do?fileType=11" style="margin-top:-11px;">模板下载</a>
								</td>
							</tr>
						</table>
						<!-- 检索  -->
									...
									<c:when test="${not empty pageList.records}">
										<c:forEach items="${pageList.records}" var="lossBonus"
											...
											<tr>
												...
												<td class="center">
												<systab:dataDictionary codeType="bonus_type" valueType="${lossBonus.type}">
