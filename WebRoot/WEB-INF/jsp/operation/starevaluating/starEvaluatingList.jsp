...
<script type="text/javascript">
	...
	//添加星级评测信息
	function saveOrUpdatestarEvaluating() {
		if($("#nav-search-input").val() != "${st.yearMonth}"){
			$("#nav-search-input").tips({
				side:3,
	            msg:'对不起，仅可以添加或修改上个月的星级评测信息',
	            bg:'#AE81FF',
	            time:2
	        });
			return false;
		}
		window.location.href = "<%=basePath%>starEvaluating/saveOrUpdatestarEvaluating.do?districtCode=" + $("#districtCode").val();
	}

	//条件查询
	function searchStarEvaluating() {
		$("#starEvaluatingForm").submit();
		$("#zhongxin").hide();
		$("#zhongxin2").show();
	}
	...
	function importOilBaseInfo() {
		bootbox.confirm("当前上传的Excel表格会自动将数据录入至上个月的数据中，请确认您上传的Excel表格是上个月的数据！", function(result) {
			if(result) {
				$("#starEvaluatingForm").attr("action", "<%=basePath%>starEvaluating/importOilBaseInfo.do");
				$("#starEvaluatingForm").submit();
			}
		});
	}
	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>starEvaluating/starEvaluatingList.do"
						  name="starEvaluatingForm" id="starEvaluatingForm" method="post" enctype="multipart/form-data">
						...
						<table>
							<tr>
								...
								<td style=""><a 
									class="btn btn-mini btn-info" onclick="searchStarEvaluating();" style="margin-top:-11px;">查询</a></td>
								<td style=""><a
									class="btn btn-mini btn-info" onclick="saveOrUpdatestarEvaluating();" style="margin-top:-11px;">星级评测</a></td>
								<td>
									<input type="file" name="uploadFile" />
									<a class="btn btn-mini btn-info" onclick="importOilBaseInfo();" style="margin-top:-11px;">导入Excel数据</a>
								</td>
								<td>
									<a class="btn btn-mini btn-info" href="<%=basePath%>common/downloadTemplate.do?fileType=3" style="margin-top:-11px;">模板下载</a>
								</td>
							</tr>
						</table>
									...
									<c:otherwise>
										<c:if test="${!empty Flag.flag}">
											...
											<c:if test="${Flag.flag == '3'}">
												<tr>
													<td colspan="100" style="text-align: center;"><font color="red">您上传的不是油站星级评测Excel文件，请确认后再上传一份正确的油站星级评测的Excel文件！</font></td>
												</tr>
											</c:if>
