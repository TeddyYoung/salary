...
<script type="text/javascript">
	...
	function importManageBase() {
		bootbox.confirm("当前上传的Excel表格会自动将数据录入至上个月的数据中，请确认您上传的Excel表格是上个月的数据！", function(result) {
			if(result) {
				$("#manageBaseForm").attr("action", "<%=basePath%>phoneBill/importPhoneBill.do");
				$("#manageBaseForm").submit();
			}
		});
	}
	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>phoneBill/phoneBillList.do"
						...
						<table>
							<tr>
								...
								<td>
									<a class="btn btn-mini btn-info" href="<%=basePath%>common/downloadTemplate.do?fileType=6" style="margin-top:-11px;">模板下载</a>
								</td>
							</tr>
						</table>
						<!-- 检索  -->
									...
									<c:when test="${not empty pageList.records}">
										<c:forEach items="${pageList.records}" var="manageBase"
											...
											<tr>
												...
												<td class="center">
													<fmt:formatNumber value="${manageBase.educationCost}" 
