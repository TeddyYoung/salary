	...
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table>
			...
			<tr>
 			...
 			<tr>
				<td>
				<select class="chzn-select" name="KHLX" id="khlx_id" data-placeholder="请选择客户类型" style="vertical-align:top;">
				<option value=""></option>
				<option value="0">总后台</option>
				<option value="1">分公司</option>
				<option value="2">一级代理商</option>
				<option value="3">二级代理商</option>
				<option value="4">三级代理商</option>
				<option value="5">终端客户</option>
				</select>
				<%-- <c:if test="总后台">selected</c:if>>${role.ROLE_NAME } --%>
				<%-- <input type="text" name="NUMBER" id="NUMBER" value="${pd.NUMBER }" maxlength="32" placeholder="这里输入编号" title="编号" onblur="hasN('${pd.USERNAME }')"/> --%>
				</td>
			</tr>
			<tr>
				<td>
				<select class="chzn-select" name="KHZT" id="khzt_id" data-placeholder="请选择客户状态" style="vertical-align:top;">
				<option value=""></option>
