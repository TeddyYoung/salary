...
<script type="text/javascript">
	...
	function addHoliday() {
		 ...
		 diag.Drag=true;
		 diag.Title ="新增补贴";
		 diag.URL = '<%=basePath%>subsiby/subsibyToAdd.do';
		 diag.Width = 800;
		 diag.Height = 200;
	...
	function updateHoliday(holidayId){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改补贴";
		 diag.URL = '<%=basePath%>subsiby/subsibyToUpdate.do?holidayId=' + holidayId;
		 diag.Width = 800;
		 diag.Height = 200;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function holidayDelete(holidayId, holidayName){
		bootbox.confirm("确定要删除[" + holidayName + "]吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>subsiby/subsibyDelete.do?holidayId=" + holidayId;
				top.jzts();
...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<form id="holidayList" action="<%=basePath%>subsiby/queryList.do" method="post">
						<table>
							<tr>
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class='center'>补贴类型</th>
									<th class='center'>补贴金额</th>
...
<%-- 											<td class='center'>${holiday.holidayName}</td> --%>
											<td class="center"><systab:dataDictionary
														codeType="subsidy_type" valueType="${holiday.type}">${dataDictionary.valuename}</systab:dataDictionary>
											</td>
											<td class='center'>
