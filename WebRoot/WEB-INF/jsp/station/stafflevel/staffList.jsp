...
<script type="text/javascript">
	...
	function toEdit(id){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改员工岗位级别";
		 diag.URL = '<%=basePath%>staffLevel/staffToEdit.do?id='+id;
		 diag.Width = 900;
		 diag.Height = 450;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
										...
										<c:forEach items="${pageList.records}" var="staff"
											...
											<tr>
												...
												<td class="center">
													...
													${obj.stationName }</biztab:biz></td>
												<td class="center"><biztab:biz type="duty"
														code="${staff.dutyCode }">${obj.dutyName }</biztab:biz></td>
												<td class="center"><systab:dataDictionary
														codeType="job_level" valueType="${staff.jobLevel}">${dataDictionary.valuename}</systab:dataDictionary>
												</td>
