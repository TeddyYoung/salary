...
<body>
	<form  action="/staff_saveOrUpdate.do" name="staffForm" id="staffForm" method="post" >
		...
		<table>
			<tr>
				<td>员工姓名：</td><td><input type="text" name="staffName" id="staffName" placeholder="这里输入员工姓名" value="" title="员工姓名"/></td>
				<td>身份证号：</td><td><input type="text" name="staffIdcard" id="staffIdcard" placeholder="这里输入身份证号" value="" title="身份证号"/></td>
			</tr>
			<tr>
				<td>联系电话：</td><td><input type="text" name="staffPhone" id="staffPhone" placeholder="这里输入联系电话" value="" title="联系电话"/></td>
				<td>员工性别：</td><td>
				<input type="radio" name="staffSex" value="0" checked="checked"/>男
				<input type="radio" name="staffSex" value="1"/>女
				</td>
			</tr>
			<tr>
				<td>员工照片：</td><td><input type="file" onchange="uploadPic()"/><img id="allUrl" src="#"/></td>
				<td>员工状态：</td><td><select name="staffStatus" id="staffStatus" class="input_txt" title="员工状态">
								<option value="0">员工状态</option>
								<c:forEach items="${statusList}" var="status">
									<option value="${status.mark}">${status.name}</option>
								</c:forEach>
							</select></td>
			</tr>
			<tr>
			...
			<tr>
				<td>员工所属职务：</td><td><select name="dutyCode" id="dutyCode" class="input_txt" title="员工所属职务">
								...
								<c:forEach items="${dutyList}" var="duty">
									<option value="${duty.dutyCode}">${duty.dutyName}</option>
								</c:forEach>
							</select></td>
			...
			<tr>
				<td>备注：</td><td><textarea rows="4" cols="68" name="remark" id="remark" placeholder="备注" title="备注"></textarea>
				</td>
			</tr>
			<tr><td>&nbsp;</td>
